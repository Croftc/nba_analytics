...
def get_rolling_stats(tdf, today_teams_list):
    ...
    tdf['Avg_3_game_OR'] = tdf.groupby(['TEAM',])['OR'].transform(lambda x: x.rolling(3).mean())
    tdf['Avg_5_game_OR'] = tdf.groupby(['TEAM',])['OR'].transform(lambda x: x.rolling(5).mean())
    tdf['Season_Avg_OR'] = tdf.groupby(['TEAM',])['OR'].transform('mean')

    tdf['Avg_3_game_3P'] = tdf.groupby(['TEAM',])['3P'].transform(lambda x: x.rolling(3).mean())
    tdf['Avg_5_game_3P'] = tdf.groupby(['TEAM',])['3P'].transform(lambda x: x.rolling(5).mean())
    tdf['Season_Avg_3P'] = tdf.groupby(['TEAM',])['3P'].transform('mean')

    tdf['Avg_3_game_3PA'] = tdf.groupby(['TEAM',])['3PA'].transform(lambda x: x.rolling(3).mean())
    tdf['Avg_5_game_3PA'] = tdf.groupby(['TEAM',])['3PA'].transform(lambda x: x.rolling(5).mean())
    tdf['Season_Avg_3PA'] = tdf.groupby(['TEAM',])['3PA'].transform('mean')

    tdf['Avg_3_game_TO'] = tdf.groupby(['TEAM',])['TO'].transform(lambda x: x.rolling(3).mean())
    tdf['Avg_5_game_TO'] = tdf.groupby(['TEAM',])['TO'].transform(lambda x: x.rolling(5).mean())
    tdf['Season_Avg_TO'] = tdf.groupby(['TEAM',])['TO'].transform('mean')

    tdf['Avg_3_game_FT'] = tdf.groupby(['TEAM',])['FT'].transform(lambda x: x.rolling(3).mean())
    tdf['Avg_5_game_FT'] = tdf.groupby(['TEAM',])['FT'].transform(lambda x: x.rolling(5).mean())
    tdf['Season_Avg_FT'] = tdf.groupby(['TEAM',])['FT'].transform('mean')
...
def update_bayesian_off_def(row, infer_df, team_ratings, team_momentum):
    row['actual_points_allowed'] = infer_df[(infer_df['GAME-ID'] == row['GAME-ID']) & (infer_df['TEAM'] != row['TEAM'])]['F'].values[0]
    # Parameters
    ...
    # Update momentum
    team_momentum[team] = momentum_decay * team_momentum.get(team, 0) + abs(delta_off + delta_def)
    team_momentum[opponent] = momentum_decay * team_momentum.get(opponent, 0) + abs(delta_off + delta_def)
...
def update_bayesian_elo_momentum(row, infer_df, team_strengths, team_momentum):
    df = infer_df
...
def parse_moneyline(row):
    try:
        ...
        raw_ml = row.find_element(By.CSS_SELECTOR, '[data-field="live-moneyline"] .data-value').text
    return -110 if raw_ml == 'even' else int(raw_ml)

def parse_spread(row):
    try:
        spread = row.find_element(By.CSS_SELECTOR, '[data-field="current-spread"] .data-value').text
    except:
        spread = row.find_element(By.CSS_SELECTOR, '[data-field="live-spread"] .data-value').text
    return float(spread)

def parse_total(row):
    try:
        total = float(''.join(c for c in row.find_element(By.CSS_SELECTOR, '[data-field="current-total"] .data-value').text if (c.isdigit() or c == '.' or c == '-')))
    except:
        total = float(''.join(c for c in row.find_element(By.CSS_SELECTOR, '[data-field="live-total"] .data-value').text if (c.isdigit() or c == '.' or c == '-')))
    return total
...
"https://www.bigdataball.com/wp-admin/admin-ajax.php?action=outofthebox-download&amp;OutoftheBoxpath=%2F11-01-2024-nba-season-player-feed.xlsx&amp;lastpath=%2F&amp;account_id=dbid:AADL0JM6TbjOPoH-7_QmtAYk4iT4-vis0Tk&amp;listtoken=1e28aa64f526af6e6fda14029e4f1bde&amp;dl=1"
def download_current_players_data(date=None):
    # Define the base URL and parameters
    base_url = "https://www.bigdataball.com/wp-admin/admin-ajax.php?action=outofthebox-download"
    account_id = "dbid:AADL0JM6TbjOPoH-7_QmtAYk4iT4-vis0Tk"
    listtoken = "1e28aa64f526af6e6fda14029e4f1bde"

    # Get current date in the required format
    current_date = datetime.now()
    yesterday = current_date - timedelta(1)
    current_date = yesterday.strftime("%m-%d-%Y") if date == None else date
    filename = f"{current_date}-nba-season-player-feed.xlsx"
    outofthebox_path = f"%2F{filename}"
